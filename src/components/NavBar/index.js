...
const NavBar = (props) => {
  ...
  if (process.env.TARO_ENV === 'alipay') {
    return (
      <View
        style={{
          height: globalStore.navHeight + globalStore.statusBarHeight,
  ...
  return (
    <View
      ...
      className={classnames(className, style.bar, { [style.noBorder]: noBorder })}
    >
      <View className={style.left} style={{ width: globalStore.headerBtnPosi.width }}>
        {left}
      </View>
      <View className={style.title}>{title}</View>
      <View className={style.right} style={{ width: globalStore.headerBtnPosi.width }}>
        {rightContent}
      </View>
    </View>
  );
